...
class LagouSpider(scrapy.Spider):
    ...
    def parse(self, response):
        """
        1、获取招聘职位的url，之后提交给scrapy解析函数进行相应字段的解析
        2、获取到下一页的Url,并交给scrapy进行进行解析
        :param response:
        :return:
        """
        for i in range(4450000, 1000000, -1):
            yield scrapy.Request(url='https://www.lagou.com/jobs/' + str(i) + '.html',
