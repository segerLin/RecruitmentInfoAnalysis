...
<script type="text/javascript">
    ...
    myChart_1.on('click', function (params) {
        var size = params.name;
        window.open('/company/size/wordCloud?size=' + size);
    });

    $.ajax("/json/job_company_size.json").done(function(data){
        // 使用刚指定的配置项和数据显示图表。
        myChart_1.setOption({
            title: {
                text: '根据公司规模(可点击查看招聘词云)',
                subtext: '下图显示的是不同公司规模和招聘数量，公司数量的二维关系',
                x:'center',
                y:'30'
            },
            legend: {},
        ...
        myChart_2.setOption({
            title: {
                text: '根据公司规模（融资阶段）进行划分',
                subtext: '图1:不同公司规模对应的热门职位\n图2:热门职位的热度，不同公司规模对应的平均工资',
                x:'center',
                y:'30'
            },
            legend: {},
